...
use epub_rs::processor::{extract_positions, extract_semantic_content, extract_text, search_chapter};
use std::io::Cursor;
use regex::Regex;
...
<body>
    ...
    for i in 0..chapter_count {
        builder = builder.add_chapter(format!("ch{}", i), format!("text/ch{}.xhtml", i), chapter_html.clone());
    }
...
fn bench_processor_extract_text(c: &mut Criterion) {
    let html = r#"<html><body>"# .to_string() + &"<p>Benchmarking extraction performance.</p>".repeat(50) + "</body></html>";
...
fn bench_processor_extract_semantic(c: &mut Criterion) {
    let html = r#"<html><body>"# .to_string() + &"<h2>Heading</h2><p>Benchmarking semantic extraction performance.</p>".repeat(50) + "</body></html>";
...
fn bench_processor_extract_positions(c: &mut Criterion) {
    let html = r#"<html><body>"# .to_string() + &"<p>Benchmarking position extraction performance.</p>".repeat(50) + "</body></html>";
...
fn bench_processor_search_chapter(c: &mut Criterion) {
    let html = r#"<html><body>"# .to_string() + &"<p>Finding the needle in the haystack.</p>".repeat(50) + "</body></html>";
    let pattern = Regex::new(r"needle").unwrap();
    
    c.bench_function("search_chapter (50 needles)", |b| {
        b.iter(|| {
            let _results = search_chapter(black_box(&html), black_box("/6/4!"), black_box(&pattern)).unwrap();
        })
    });
}

...
mod tests {
    ...
    use epub_rs::model::{Creator, Metadata, TocEntry};
...
<script type="text/javascript"></script>
        ...
        assert_eq!(
            ...
            vec!["Technology".to_string(), "Rust".to_string()]
        );
        ...
        let extracted_stream = archive
            .get_resource_by_id(&book, "stream1")
            .expect("Failed to get stream resource");
        ...
        let extracted_ncx = archive
            .get_resource_by_id(&book, "ncx")
            .expect("Failed to get ncx");
        ...
        let extracted_nav = archive
            .get_resource_by_id(&book, "nav")
        ...
        assert!(
            ncx_str
                .contains("<pageTarget id=\"page-1\" type=\"normal\" value=\"1\" playOrder=\"3\">")
        );
        assert!(
            ncx_str
                .contains("<pageTarget id=\"page-2\" type=\"normal\" value=\"2\" playOrder=\"4\">")
        );

        // Extract and verify content
        let extracted_css = archive
            .get_resource_by_id(&book, "style.css")
            .expect("Failed to get css");
        assert_eq!(extracted_css, css_content);

        let extracted_html = archive
            .get_resource_by_id(&book, "chapter1")
            .expect("Failed to get html");
        assert_eq!(extracted_html, chapter1_html);

        let extracted_text =
            epub_rs::processor::extract_text(&extracted_html).expect("Failed to extract text");
    ...
    fn test_generate_epub_v2() {
        ...
        let builder = EpubBuilder::new()
            ...
            .add_chapter(
                "chapter1",
                "text/ch1.xhtml",
                b"<html><body>Legacy</body></html>".to_vec(),
            );

        let mut buffer = Cursor::new(Vec::new());
        builder
            .generate(&mut buffer)
            .expect("Failed to generate V2 EPUB");
    ...
    fn test_fixed_layout_generation() {
        use epub_rs::model::{EpubVersion, LayoutType, Metadata, PageSpread};
        ...
        let builder = EpubBuilder::new()
            ...
            .add_chapter_with_layout(
                "page1",
                "text/p1.xhtml",
                b"<html><body>Page 1</body></html>".to_vec(),
                None,
            ...
            .add_chapter_with_layout(
                "page2",
                "text/p2.xhtml",
                b"<html><body>Page 2</body></html>".to_vec(),
                Some(LayoutType::Reflowable),
        ...
        builder
            .generate(&mut buffer)
            .expect("Failed to generate FXL EPUB");
